...
Java boat control is split across `MOVE_VEHICLE`, `PADDLE_BOAT`, and the normal shift input edge. Translate vehicle position/rotation into the tracked mounted entity, add Bedrock's boat network Y offset, convert Java yaw to Bedrock vehicle yaw with the verified quarter-turn offset, and set `IsInClientPredictedVehicle`, paddle flags, optional vehicle rotation, and the signed-varlong predicted runtime ID in `PlayerAuthInput`. Send `StopRiding` once on the dismount edge and reset cached vehicle/paddle state when Bedrock removes the mount. Re-audit packet fields and offsets for every Java and Bedrock release; cover steering in both directions, forward/back, dismount, and re-entry on a real host.
...
Survival double-click consolidation arrives as `PICKUP_ALL` after the ordinary pickup has emptied the clicked slot. Serialize both packets through the request tracker, then require a nonempty non-full cursor and an empty clicked slot. Scan the complete Java menu in button direction, including the open container and player inventory but excluding crafting outputs; match Java's two passes by taking partial matching stacks before full stacks. Emit one Bedrock `Take` per source into the cursor, stop at the real maximum stack size, and assign the negative request ID to the optimistic source remainder and cursor after every action so the next `Take` references the current predicted ID. Snapshot every touched container, refresh crafting output when a player or table grid may have changed, and add a multi-`Take` wire test proving sequential cursor IDs and zero trailing bytes.
...
Java's creative screen owns its carried item locally and represents moves as authoritative `SET_CREATIVE_MODE_SLOT` updates, not a Bedrock HUD-cursor transaction. Mark these requests as Java-client-managed. On success, apply response corrections and authoritative stack network IDs to the ViaBedrock tracker without sending a full Java container refresh; its Bedrock HUD cursor is empty and would erase the item from Java's mouse between the source and destination updates, producing disappearing items and apparent duplication. On rejection, keep the ordinary snapshot rollback, full Java refresh, and Bedrock mismatch resync so a failed prediction cannot become a real duplication. Do not suppress refreshes for survival clicks, crafting, or other cursor-backed requests.

The same cursor ownership rule applies to unsolicited Bedrock `InventorySlot` and `InventoryContent` updates for the player's inventory, armor, and offhand while Java is in creative mode. A full Java content refresh carries Bedrock's empty HUD cursor and can erase armor from the mouse when it is removed. Update the tracker, then publish only the corrected Java slots while preserving Java's local carried item. Keep full-content refreshes for survival and rejection rollback. Validate equip and removal in all four Java armor slots, cursor placement, dropping, and rapid moves; never infer success solely from the Bedrock equipment appearance.

Bedrock's offhand has three distinct slot identities: Java's player menu uses slot 45, Bedrock `InventoryContent`/`InventorySlot` stores the single item at index 0, and `ItemStackRequest` plus its response addresses `OffhandContainer` as slot 1. Keep explicit storage-to-request and response-to-storage mappings; using slot 0 in a request rejects even a valid totem, while applying response slot 1 directly to the one-element storage array loses the authoritative stack network ID. Decode the response's first byte as `slot` and the next byte as `hotbarSlot`; do not accidentally use the adjacent hotbar field as the corrected slot. Route Java's swap-with-offhand key through the same serialized request tracker as clicks. A 1.26.40 LAN host rejects both a direct hotbar-to-offhand `Place` and a hotbar-to-cursor `Take` with `FailedToValidateDstSlot` when no Bedrock inventory is open. For F outside a Java screen, send `Interact/OpenInventory`, wait for the host's `ContainerOpen` of type `INVENTORY`, submit one direct `Place` or `Swap`, and close that synthetic Bedrock inventory only after its stack response. Debounce repeated F packets while the open or request is pending and clear a missing-open request after a bounded timeout. Do not expose the synthetic open as a Java screen. Request-local predictions must update the inventory mirror without firing `MobEquipment` or other equipment side effects before the host sees the `ItemStackRequest`; otherwise removing the predicted offhand first makes its source stack-network ID stale and produces `FailedToValidateSrcSlot`. Publish equipment only from the accepted correction or rejection rollback. Let the Bedrock host enforce `minecraft:allow_off_hand` and wearable restrictions so behavior-pack items remain supported; on `CannotPlaceItem` or another rejection, restore the snapshots instead of maintaining a static vanilla allowlist or bypassing the restriction. Survival rollback uses a full Java refresh. A Java-client-managed creative rejection must restore affected slots individually so Bedrock's empty HUD cursor does not erase the rejected item from Java's mouse. Do not emulate Java offhand use for items Bedrock can carry but cannot actively use.
...
Bedrock often maps the generic `minecraft:bed` state to a Java fallback colour and supplies the real colour in its block entity. Resolve the coloured Java state semantically by changing the bed identifier while preserving facing/part/occupied properties; do not depend on registry-ID arithmetic. Reapply the retained bed block entity even when Bedrock resends the same numeric palette state during sleep/wake, otherwise the fallback (commonly white) can leak visually. Test multiple colours, both halves, sleep/wake, a neighboring update, and chunk reload.
