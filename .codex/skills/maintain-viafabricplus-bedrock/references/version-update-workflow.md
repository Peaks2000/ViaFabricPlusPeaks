...
For server-authoritative item pickups, let `InventorySlot` update the tracked Bedrock item before refreshing Java state. Bedrock's `TakeItemActor` identifies only the collected and collecting actors, so it cannot safely determine the destination slot after stacking. Translate player-inventory, armor, and offhand slot corrections into a combined Java player `CONTAINER_SET_CONTENT`; this keeps the hotbar and every player-inventory view synchronized even while another screen is open. Cancel the mapped slot wrapper and send the combined content as a fresh Java packet. Re-typing the in-flight `CONTAINER_SET_SLOT` wrapper can leave the original mapping active in part of the protocol pipeline, so the client may discard the full-content payload even though ViaBedrock's tracker is correct. Preserve the specialized cursor, crafting-table, and non-player-container paths.

Use placement as a useful differential test: if a picked-up item becomes visible only after placing or otherwise changing a slot, the authoritative tracker likely contains the item and the immediate clientbound refresh was not delivered. Verify the fresh-packet path before adding actor-based destination guesses. Treat `InventorySlot outside PLAY state` during initial configuration as a separate baseline-loss problem; it does not explain a later pickup packet handled after the join reaches `PLAY`.
