...
- `Inventory request ... failed: Error` after selecting a crafting result on protocol 2168: verify that `CraftingData` shaped/shapeless arrays were retained and that the request sends `CraftRecipe`, `CraftResults`, input `Consume` actions, then a transfer from `CreatedOutputContainer` slot 50. The mapped action IDs are 10 and 17 while the repeated Cereal discriminators remain 12 and 19.
