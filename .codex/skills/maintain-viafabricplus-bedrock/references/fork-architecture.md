...
- `Packet violation warning: PacketMalformed`, `Violating Packet: PlayerAuthInputPacket`, `invalid enum value`, and `packetId: 144` immediately after the player briefly appears in an iOS world: protocol 2168's `PlayerAuthInputPacket` has no legacy presence booleans. In `ClientPlayerPackets`, write the input-data bitset directly after head yaw; emit item-use, item-stack-request, block-action, and vehicle payloads only when their input-data flags are set. Preserve the protocol-2168 unsigned-varint interaction model, signed-varint block-action count, and the no-data `StopDestroyBlock` branch.
